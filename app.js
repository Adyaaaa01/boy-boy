...
const sb = HAS_SUPABASE_CONFIG ? supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY) : null;

function demoSeed(){
  return {
    authUser:{id:"demo-admin", email:"Adyaasd1@gmail.com", user_metadata:{name:"Администратор"}},
    profile:{id:"demo-admin", family_id:"demo-family", name:"Администратор", email:"Adyaasd1@gmail.com", role:"admin"},
    family:{id:"demo-family", name:"Манай гэр бүл"},
    members:[
      {id:"demo-admin", family_id:"demo-family", name:"Администратор", email:"Adyaasd1@gmail.com", role:"admin"},
      {id:"demo-mom", family_id:"demo-family", name:"Ээж", email:"mom@example.com", role:"member"},
      {id:"demo-dad", family_id:"demo-family", name:"Аав", email:"dad@example.com", role:"member"}
    ],
    tx:[
      {id:"t1",family_id:"demo-family",member_id:"demo-admin",type:"income",amount:3500000,category:"Цалин",date:"2026-05-04",note:"Сарын цалин"},
      {id:"t2",family_id:"demo-family",member_id:"demo-dad",type:"income",amount:800000,category:"Бизнес",date:"2026-05-12",note:"Бизнесийн орлого"},
      {id:"t3",family_id:"demo-family",member_id:"demo-mom",type:"expense",amount:450000,category:"Хоол хүнс",date:"2026-05-08",note:"Хоол хүнсний зардал"},
      {id:"t4",family_id:"demo-family",member_id:"demo-admin",type:"expense",amount:300000,category:"Боловсрол",date:"2026-05-10",note:"Сургалт"},
      {id:"t5",family_id:"demo-family",member_id:"demo-dad",type:"expense",amount:120000,category:"Унаа",date:"2026-05-13",note:"Шатахуун"}
    ],
    savings:[
      {id:"s1",family_id:"demo-family",name:"Аялал",target_amount:5000000,current_amount:3000000,monthly_rate:1.2,months:12,reminder_day:1},
      {id:"s2",family_id:"demo-family",name:"Хүүхдийн фонд",target_amount:10000000,current_amount:2000000,monthly_rate:1.4,months:24,reminder_day:5}
    ],
    loans:[
      {id:"l1",family_id:"demo-family",name:"Орон сууцны зээл",total_amount:50000000,paid_amount:6000000,monthly_payment:500000,annual_rate:12,remaining_months:729,start_date:"2025-01-01",end_date:"2087-03-01"}
    ]
  }
}
...
async function sendMagicOtp(){
  if(!HAS_SUPABASE_CONFIG) return toast("Demo mode дээр OTP имэйл явуулахгүй. Supabase key оруулсны дараа ажиллана.");
...
async function forgotPassword(){
  if(!HAS_SUPABASE_CONFIG) return toast("Demo mode дээр password reset имэйл явуулахгүй. Supabase key оруулсны дараа ажиллана.");
...
async function registerAdmin(){
  if(!HAS_SUPABASE_CONFIG){
    const d = demoSeed();
    d.profile.name = $('regName').value || "Администратор";
    d.profile.email = $('regEmail').value || "Adyaasd1@gmail.com";
    d.authUser.email = d.profile.email;
    localStorage.setItem("familyFinanceDemo", JSON.stringify(d));
    await boot();
    return;
  }
...
async function logout(){
  if(HAS_SUPABASE_CONFIG) await sb.auth.signOut();
  location.reload();
}

async function boot(){
  if(!HAS_SUPABASE_CONFIG){
    const d = JSON.parse(localStorage.getItem("familyFinanceDemo") || "null") || demoSeed();
    authUser = d.authUser; profile = d.profile; family = d.family;
    state.tx = d.tx; state.savings = d.savings; state.loans = d.loans; state.members = d.members;
    $('authScreen').classList.add('hidden'); $('app').classList.remove('hidden'); $('mobilebar').classList.remove('hidden');
    $('sideUser').textContent = profile.name; $('sideEmail').textContent = profile.email + " · DEMO";
    attachNav(); page('home');
    toast("Demo mode: Supabase key хийх хүртэл дата энэ browser дээр хадгалагдана.");
    return;
  }
...
async function loadAll(){
  if(!HAS_SUPABASE_CONFIG){
    const d = JSON.parse(localStorage.getItem("familyFinanceDemo") || "null") || demoSeed();
    state.tx=d.tx; state.savings=d.savings; state.loans=d.loans; state.members=d.members;
    profile=d.profile; family=d.family; authUser=d.authUser;
    return;
  }
...
function persistDemo(){
  if(!HAS_SUPABASE_CONFIG){
    localStorage.setItem("familyFinanceDemo", JSON.stringify({authUser,profile,family,members:state.members,tx:state.tx,savings:state.savings,loans:state.loans}));
  }
}
const render = {
home(){
...
async function addTx(type){
  ...
  if(!HAS_SUPABASE_CONFIG){
    row.id = "t"+Date.now(); state.tx.unshift(row); persistDemo(); closeM(); render[state.page](); return;
  }
...
async function addSaving(){
  ...
  if(!HAS_SUPABASE_CONFIG){
    row.id = "s"+Date.now(); state.savings.unshift(row); persistDemo(); closeM(); render.savings(); return;
  }
...
async function addLoan(){
  ...
  if(!HAS_SUPABASE_CONFIG){
    row.id = "l"+Date.now(); state.loans.unshift(row); persistDemo(); closeM(); render.loans(); return;
  }
...
async function inviteMember(){
  ...
  if(!HAS_SUPABASE_CONFIG){
    state.members.push({id:"m"+Date.now(), family_id:profile.family_id, name, email, role});
    persistDemo(); closeM(); render.members(); toast("Demo mode: гишүүн нэмэгдлээ. OTP Supabase холбосны дараа ажиллана."); return;
  }
...
async function saveProfile(){
  if(!HAS_SUPABASE_CONFIG){
    profile.name = $('setName').value; persistDemo(); $('sideUser').textContent=profile.name; toast("Хадгаллаа"); return;
  }
...
async function clearDemo(){
  ...
  if(!HAS_SUPABASE_CONFIG){
    localStorage.removeItem("familyFinanceDemo"); location.reload(); return;
  }
...
if(HAS_SUPABASE_CONFIG){ sb.auth.onAuthStateChange((_event, session)=>{ if(session?.user && !authUser) boot(); }); }
boot();
